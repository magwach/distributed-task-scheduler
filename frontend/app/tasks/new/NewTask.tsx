      ...
      <form onSubmit={handleSubmit}>
        <div className="form-card">
          ...
          <div className="form-group">
            ...
            />
            {errors.schedule && <div className="form-error">⚠ {errors.schedule}</div>}
            ...
            <div style={{ marginTop: 14, display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {CRON_EXAMPLES.map((ex) => (
                <button
                  ...
                  style={{
                    background: form.schedule === ex.value ? 'var(--accent-cyan-dim)' : 'var(--bg-secondary)',
                    border: `1px solid ${form.schedule === ex.value ? 'rgba(0,212,255,0.3)' : 'var(--border)'}`,
                    borderRadius: 'var(--radius-sm)',
                    padding: '6px 12px',
                    fontSize: '11px',
                    fontFamily: 'var(--font-mono)',
                    color: form.schedule === ex.value ? 'var(--accent-cyan)' : 'var(--text-secondary)',
                    cursor: 'pointer',
